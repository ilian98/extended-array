...
mod tests {
    ...
    fn check_exrays(
        exrays: &ExrayMap<i64, i64>,
        expected: Vec<(String, Vec<&i64>, Vec<String>)>,
        functions: &FuncMap<i64, i64>,
    ) -> bool {
    ...
    fn test_create() {
        ...
        assert_match!(
            create(
                &[String::from("create"), String::from("test_create")],
                &mut line_it,
                &mut exrays,
                &functions
            )
        ...
        assert_match!(
            create(&[], &mut line_it, &mut exrays, &functions).err(),
            Some(CommandError::CreateError(_))
        );
        assert_match!(
            create(
        ...
        assert_match!(
            create(
                &[String::from("create"), String::from("no-file")],
                &mut line_it,
                &mut exrays,
                &functions
            )
        ...
        assert_eq!(
            check_exrays(
                ...
                vec![
                    ...
                    (String::from("test2"), vec![&5], vec![String::from("min")])
                ],
                &functions
            ),
            true
    ...
    fn test_save() {
        ...
        assert_match!(
            create(
                &[String::from("create"), String::from("saved")],
                &mut line_it,
                &mut exrays,
                &functions
            )
    ...
    fn test_get_element() {
        ...
        exrays.insert(
            String::from("test"),
            Exray::<i64, i64>::new(numbers.clone(), vec![]),
        );
        for i in 0..numbers.len() {
            assert_eq!(
                get_element(
                    &[String::from("1"), String::from("test"), i.to_string()],
                    &exrays
                )
                .unwrap(),
                &numbers[i]
        ...
        assert_match!(
            get_element(
                &[String::from("1"), String::from("no-exray"), 0.to_string()],
                &exrays
            )
            .err(),
            Some(CommandError::GetElementError(_))
        ...
        assert_match!(
            get_element(
                &[String::from("1"), String::from("test"), (-1).to_string()],
                &exrays
            )
            .err(),
            Some(CommandError::GetElementError(_))
        ...
        assert_match!(
            get_element(
                &[String::from("1"), String::from("test"), 6.to_string()],
                &exrays
            )
            .err(),
            Some(CommandError::GetElementError(_))
    ...
    fn test_change_element() {
        ...
        exrays.insert(
            String::from("test"),
            Exray::<i64, i64>::new(numbers.clone(), vec![*functions.get("sum").unwrap()]),
        );
        for i in 0..numbers.len() {
            assert_eq!(
                change_element(
            ...
            assert_eq!(
                exray_fvalues(
                    &[String::from("1"), String::from("test")],
                    &mut exrays,
                    &functions
                )
                .unwrap(),
                vec![(String::from("sum"), &sum)]
            );
        }

        assert_match!(
            change_element(&[], &mut exrays).err(),
    ...
    fn test_insert_element() {
        ...
        assert_eq!(
            exrays.get("test").unwrap().to_vec(),
            vec![&0, &2, &1, &9, &2, &-5, &3, &10, &4, &1024, &5]
        );

        assert_match!(
            insert_element(&[], &mut exrays).err(),
    ...
    fn test_erase_element() {
        ...
        assert_match!(
            erase_element(
                &[String::from("1"), String::from("test"), 5.to_string()],
                &mut exrays
            )
            .err(),
            Some(CommandError::ExrayError(ExrayError::IndexError(_)))
        ...
        for ind in erase_indices {
            assert_eq!(
                erase_element(
                    &[String::from("1"), String::from("test"), ind.to_string()],
                    &mut exrays
                )
                .unwrap(),
                ()
    ...
    fn test_erase_segment() {
        ...
        assert_eq!(exrays.get("test").unwrap().to_vec(), Vec::<&i64>::new());

        assert_match!(
            erase_segment(&[], &mut exrays).err(),
    ...
    fn test_extract_or_clone_segment() {
        ...
        assert_eq!(
            exrays.get("clone").unwrap().to_vec(),
            vec![&3, &4, &5, &6, &7]
        );
        ...
        assert_match!(
            extract_or_clone_segment(&[], &mut exrays, String::from("clone")).err(),
    ...
    fn test_insert_exray() {
        ...
        exrays.insert(
            String::from("test2"),
            Exray::<i64, i64>::new(vec![4, 5, 6, 7], vec![]),
        );
    ...
    fn test_segment_fvalues() {
        ...
        exrays.insert(
            ...
            Exray::<i64, i64>::new(
                numbers.clone(),
                vec![
                    *functions.get("sum").unwrap(),
                    *functions.get("max").unwrap(),
                    *functions.get("min").unwrap(),
                ],
            ),
        );
        exrays.insert(
        ...
        assert_match!(
            segment_fvalues(&[], &mut exrays, &functions).err(),
    ...
    fn test_exray_fvalues() {
        ...
        assert_eq!(
            exray_fvalues(
                &[String::from("1"), String::from("test2")],
                &exrays,
                &functions
            )
            .unwrap(),
        ...
        assert_match!(
            exray_fvalues(&[], &exrays, &functions).err(),
        ...
        assert_match!(
            exray_fvalues(
                &[String::from("1"), String::from("no-exray")],
                &exrays,
                &functions
            )
            .err(),
...
fn try_line(result: Option<Result<String, Error>>) -> Result<Vec<String>, CommandError> {
    match result {
        None => {
            return Err(CommandError::InputEnd(String::from(
                "Input ended without reading the numbers",
            )))
        }
        Some(line) => {
            if line.is_err() {
    ...
    <T as FromStr>::Err: std::fmt::Debug,
{
    if words.len() != 1 && words.len() != 2 {
        return Err(CommandError::CreateError(String::from(
            "Zero or only one argument expected",
        )));
    }
    ...
    T: Display,
{
    match check_name(words, 3, exrays) {
        Err(None) => {
            return Err(CommandError::SaveError(String::from(
                "Two arguments expected - name of exray and name of file for saving",
            )));
        }
    ...
    T: Display,
{
    match check_name(words, 2, exrays) {
        Err(None) => {
            return Err(CommandError::PrintError(String::from(
                "One arguments expected - name of exray",
            )));
        }
...
fn get_element<'a, T, U>(
    ...
    match check_name(words, 3, exrays) {
        Err(None) => {
            return Err(CommandError::GetElementError(String::from(
                "Two arguments expected - name of exray and index of element",
            )));
        }
    ...
    let index = match words[2].parse::<usize>() {
        Err(_) => {
            return Err(CommandError::GetElementError(String::from(
                "Index cannot be parsed as usize",
            )))
        }
        Ok(num) => num,
    };
    ...
    if index >= exray.len() {
        return Err(CommandError::GetElementError(String::from(
            "Index is greater than last index",
        )));
    }
    ...
    T: FromStr,
{
    match check_name(words, 4, exrays) {
        Err(None) => {
            return Err(CommandError::ChangeElementError(String::from(
                "Three arguments expected - name of exray, index of element and new value",
            )));
        }
    ...
    T: FromStr,
{
    match check_name(words, 4, exrays) {
        Err(None) => {
            return Err(CommandError::InsertElementError(String::from(
                "Three arguments expected - name of exray, index of inserted element and value",
            )));
        }
...
fn erase_element<T, U>(words: &[String], exrays: &mut ExrayMap<T, U>) -> Result<(), CommandError> {
    match check_name(words, 3, exrays) {
        Err(None) => {
            return Err(CommandError::EraseElementError(String::from(
                "Two arguments expected - name of exray and index of erased element",
            )));
        }
...
fn erase_segment<T, U>(words: &[String], exrays: &mut ExrayMap<T, U>) -> Result<(), CommandError> {
    ...
    let beg_index = match words[2].parse::<usize>() {
        Err(_) => {
            return Err(CommandError::EraseSegmentError(String::from(
                "Begin index cannot be parsed as usize",
            )))
        }
        Ok(num) => num,
    };
    let end_index = match words[3].parse::<usize>() {
        Err(_) => {
            return Err(CommandError::EraseSegmentError(String::from(
                "End index cannot be parsed as usize",
            )))
        }
        Ok(num) => num,
    };
    ...
    U: Clone,
{
    ...
    let beg_index = match words[2].parse::<usize>() {
        Err(_) => {
            return Err(CommandError::ExtractCloneSegmentError(String::from(
                "Begin index cannot be parsed as usize",
            )))
        }
        Ok(num) => num,
    ...
    if exrays.contains_key(&new_name) {
        return Err(CommandError::ExtractCloneSegmentError(String::from(
            "Exray with the destination name already exists",
        )));
    }
...
fn insert_exray<T, U>(
    ...
    if exrays.contains_key(&name_dest) == false {
        return Err(CommandError::InsertExrayError(String::from(
            "No exray with the destination name",
        )));
    }
    if words[1] == name_dest {
        return Err(CommandError::InsertExrayError(String::from(
            "Exray source and destination name are the same",
        )));
    }
...
fn segment_fvalues<T, U>(
    ...
    exrays: &mut ExrayMap<T, U>,
    functions: &FuncMap<T, U>,
) -> Result<Vec<(String, U)>, CommandError>
where
    U: Clone,
{
    match check_name(words, 4, exrays) {
    ...
    let beg_index = match words[2].parse::<usize>() {
        Err(_) => {
            return Err(CommandError::SegmentFvaluesError(String::from(
                "Begin index cannot be parsed as usize",
            )))
        }
        Ok(num) => num,
...
fn exray_fvalues<'a, T, U>(
    ...
    exrays: &'a ExrayMap<T, U>,
    functions: &FuncMap<T, U>,
) -> Result<Vec<(String, &'a U)>, CommandError> {
    match check_name(words, 2, exrays) {
        Err(None) => {
            return Err(CommandError::ExrayFvaluesError(String::from(
                "One argument expected - name of exray",
            )));
        }
...
fn exray_len<T, U>(
    ...
    match check_name(words, 2, exrays) {
        Err(None) => {
            return Err(CommandError::ExrayLenError(String::from(
                "One argument expected - name of exray",
            )));
        }
